...
export default function Home() {
  ...
  async function runAnalysis(): Promise<any> {
    ...
    return JSON.parse(clean); // may throw — caller decides whether to retry
  }
  ...
  async function analyze() {
    ...
    try {
      ...
      try {
        ...
        const isParseError = firstErr instanceof SyntaxError || /JSON/i.test(firstErr.message || "");
        if (!isParseError) throw firstErr; // real errors (network, server) shouldn't retry silently
        // One automatic retry on malformed JSON — generation glitches are usually transient
        data = await runAnalysis();
      }
