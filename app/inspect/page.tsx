    ...
    <div style={{ minHeight: "100vh", background: C.bg, padding: "32px 20px", paddingBottom: 110 }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
          ...
          <button
            onClick={() => { setResult(null); setPrompt(""); setDetailFailed(false); }}
            style={{ background: "none", border: "1px solid " + C.border, color: C.muted, padding: "8px 16px", borderRadius: 8, fontSize: 13, cursor: "pointer" }}
          >
            New Analysis
          </button>
        </div>
        ...
        {detailFailed && (
          <div style={{ marginBottom: 20, padding: "10px 16px", background: C.orange + "0D", border: "1px solid " + C.orange + "25", borderRadius: 8, color: C.orange, fontSize: 13 }}>
            Detailed breakdown timed out. Your scores above are complete and accurate.
          </div>
        )}
              ...
              Object.entries(result.aiCompatibility).map(([ai, val]: any) => (
                <div key={ai} style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ height: 6, background: C.border, borderRadius: 3 }}>
                    <div style={{ height: "100%", width: clamp(val, 0, 100) + "%", background: scoreColor(val / 10), borderRadius: 3 }} />
                  </div>
                </div>
              ))
            )}
